...
DF_VARIABLES  = ["F_H2O", "F_CO2", "F_GPP", "SIF683", "SIF740", "SIF757", "SIF771", "NDVI", "EVI", "NIRv","g_lw_un_pred","T_VEG_un_pred"];
...
function run_time_step!(spac::SPACMono{FT}, dfr::DataFrameRow, beta::BetaGLinearPsoil{FT}) where {FT<:AbstractFloat}
    ...
    dfr.g_lw_un_pred =g_lw_pred
    dfr.T_VEG_un_pred=t_veg_pred
...
# the actual test !
@time run_model!(spac, wddf, joinpath(@__DIR__, "debug.output_Hyb_oz.nc"));
