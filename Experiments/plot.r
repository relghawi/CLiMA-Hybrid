...
p2<- p+ stat_poly_eq(formula = my.formula,data =df, aes(y=T_VEG,x =T_VEG_pred,label = paste(..adj.rr.label.., sep = "~~~")),rr.digits=3,size=3, parse = TRUE,label.x = "right", label.y = "bottom")                  
ggsave(plot = p2, width =5, height = 3, dpi = 500, filename = "/Net/Groups/BGI/scratch/relghawi/paper_2/GPP_siteID_ICON.png")
...
df<- read.csv("/Net/Groups/BGI/people/relghawi/Julia_hyb/Clima-Hyb2/Experiments/predictions3_gc_oz.csv")
...
#    geom_density_2d(data = df, aes(y=gs,x =α),alpha=0.3,bins=6)+
    ...
    axis.text.x=element_text(size=8),axis.text.y=element_text(size=8),legend.title = element_text(size=10),
    axis.title = element_text(size=10),plot.title =element_text(size=15),plot.margin=unit(c(0.3,0.3,0.3,0.3), "cm")) 
p2<- p + scale_x_continuous(guide = "prism_minor",breaks= seq(0,0.08,1e-2), 
                  limits = c(0,0.08), expand = c(0,0), minor_breaks = seq(0,0.08,5e-3))+
                  scale_y_continuous(guide = "prism_minor",breaks= seq(0,0.08,1e-2), 
                  limits = c(0,0.08), expand = c(0,0), minor_breaks = seq(0,0.08,5e-3))+ geom_abline(slope=1,inter=0)
