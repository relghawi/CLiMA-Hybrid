...
predictors = [:T_AIR, :RAD, :SWC_1,:LAIx, :p_sat,:p_H2O,:p_atm,:LA,:WIND,:vpd]
target = :T_VEG_un
x = [:LAIx, :p_sat,:p_H2O,:p_atm,:LA] # Assuming as independent variables

trainloader, valloader, testloader, trainall, d_train, d_vali, d_test = split_data(df, target, predictors, x, batchsize=50, f=0.8, v=0.1, shuffle=true, partial=true)


function train(model, trainloader::Flux.Data.DataLoader, valloader::Flux.Data.DataLoader,
        checkpoint_file::String; optimizer=Flux.ADAM(0.001), n_epochs=5000, patience=500)
